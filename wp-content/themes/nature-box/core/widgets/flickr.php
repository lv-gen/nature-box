...
class wpshower_flickr extends WP_Widget {
	...
	function wpshower_flickr() {
		/* Widget settings. */
		$widget_args = array( 'classname' => 'unspoken-flickr', 'description' => 'Показать Flickr Фотопоток' );

		/* Create the widget. */
		$this->WP_Widget( false, 'Unspoken - Flickr', $widget_args, array( 'width' => 270));
	}
	...
	function widget( $args, $instance ) {
        ...
        if ( empty($instance) ) {
            $instance = array(
                'title' => __( 'Flickr фотографии', 'unspoken' ),
                'type' => '',
                'tags' => '',
                'set' => '',
                'id' => '',
		...
		$after_list = '</ul>';
        $default_title = __( 'Неназванная фотография Flickr', 'unspoken' );
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'num_items' ); ?>">Показать</label>
			...
			<select onchange="javascript: toggleSource('<?php echo $this->get_field_id( 'type' ); ?>');" name="<?php echo $this->get_field_name( 'type' ); ?>" id="<?php echo $this->get_field_id( 'type' ); ?>">
				<option <?php if($instance['type'] == 'user') { echo 'selected'; } ?> value="user">юзер</option>
				<option <?php if($instance['type'] == 'set') { echo 'selected'; } ?> value="set">множество</option>
				<option <?php if($instance['type'] == 'favorite') { echo 'selected'; } ?> value="favorite">любимые</option>
				<option <?php if($instance['type'] == 'group') { echo 'selected'; } ?> value="group">группа</option>
				<option <?php if($instance['type'] == 'public') { echo 'selected'; } ?> value="public">сообщество</option>
			</select>
			фотографии.
		</p>
		<p class="id_parent">
			<label for="<?php echo $this->get_field_id( 'id' ); ?>">ИД юзера или группы</label>
			<input name="<?php echo $this->get_field_name( 'id' ); ?>" type="text" id="<?php echo $this->get_field_id( 'id' ); ?>" value="<?php echo $instance['id']; ?>" size="20" /><a
                href="http://idgettr.com/" rel="external nofollow" target="_blank">Каков мой ИД?</a>
		</p>
		<p class="set_parent">
			<label for="<?php echo $this->get_field_id( 'set' ); ?>">ИД Множества</label>
			...
			<small>Используйте числа из множества URL</small>
		</p>
		<p class="tags_parent">
			<label for="<?php echo $this->get_field_id( 'tags' ); ?>">Метки (необязательна)</label>
			...
			<small>Разделенные запятыми без пробелов</small>
		</p>
	...
	function getRSS($settings) {
		...
		elseif ($settings['type'] == "public" || $settings['type'] == "community") { $rss_url = 'http://api.flickr.com/services/feeds/photos_public.gne?tags=' . $settings['tags'] . '&format=rss_200'; }
			...
			print '<strong>Не было установлено параметр "типа" Проверьте свои настройки, или обеспечьте параметр как аргумент.</strong>';
		}
		...
		if ( $feed->error() )
			printf ('Была ошибка при подключении к серверу, пожалуйста, попробуйте еще раз!');
