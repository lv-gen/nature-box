...
class wpshower_twitter extends WP_Widget {
    ...
    function wpshower_twitter() {
        $widget_args = array('classname' => 'unspoken-twitter', 'description' => __('Список Ваших последных твитов', 'unspoken') );
        parent::WP_Widget(false, __('Unspoken - Twitter', 'unspoken'), $widget_args);
    }
    ...
    function form($instance) {
        ...
        ?>
            ...
            <p><label for="unspoken_twittername"><?php _e( 'Twitter имя:', 'unspoken' ); ?></label><input class="widefat" type="text" name="unspoken_twittername" id="unspoken_twittername" value="<?php echo get_option('unspoken_twittername'); ?>"/></p>
            <p><label for="unspoken_updates"><?php _e( 'Количество твитов (максимум 20):', 'unspoken' ); ?><br /></label><input type="text" name="unspoken_updates" id="unspoken_updates" size="3" value="<?php echo (get_option('unspoken_updates')) ? get_option('unspoken_updates') : 5 ; ?>"/></p>
            <p><label for="unspoken_period"><?php _e( 'Кэш твитов:', 'unspoken' ); ?><br /></label><input type="text" name="unspoken_period" id="unspoken_period" size="3" value="<?php echo (get_option('unspoken_period')) ? get_option('unspoken_period') : 5 ; ?>"/> <?php _e( 'минуты', 'unspoken' ); ?></p>
        <?php
    }
    ...
    function widget($args, $instance) {
        ...
        if ( !$twittername) $twittername = 'geckonchik';
        if ( $title ) {
