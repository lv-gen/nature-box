...
class wpshower_recent_posts extends WP_Widget {
    ...
    function wpshower_recent_posts() {
        $widget_args = array('classname' => 'unspoken_recent_posts', 'description' => __('Последние записи с миниатюрами', 'unspoken') );
        parent::WP_Widget(false, __('Unspoken - Последние записи', 'unspoken'), $widget_args);
    }

    function form($instance) {
        $defaults = array(
            'title' => __( 'Последние записи', 'unspoken' ),
            'number' => 5,
            'hide_thumb' => false
        );
        ...
        <p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e( 'Количество записей:', 'unspoken' ); ?><input size="3" id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" /></label></p>
        <p><input class="checkbox" id="<?php echo $this->get_field_id('hide_thumb'); ?>" name="<?php echo $this->get_field_name('hide_thumb'); ?>" type="checkbox" <?php echo $hide_thumb; ?> />  <label for="<?php echo $this->get_field_id('hide_thumb'); ?>"><?php _e( 'Отключить миниатюры', 'unspoken' ); ?></label></p>
    ...
    function update( $new_instance, $old_instance ) {
        ...
        $instance['title'] = strip_tags($new_instance['title']) ? strip_tags($new_instance['title']) : __( 'Последние записи', 'unspoken' );
