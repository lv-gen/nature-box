...
class wpshower_getconnected extends WP_Widget {
    ...
    function wpshower_getconnected() {
        $widget_args = array('classname' => 'unspoken-getconnected', 'description' => __('Список с Вашими социальными ссылками', 'unspoken') );
        parent::WP_Widget(false, __('Unspoken - Связи', 'unspoken'), $widget_args);
    }
            ...
            foreach ( $instance_items as $key => $value ) : ?>
                <div class="getconnected-item">
                    ...
                    <p class="getconnected-options">
                        <label for="<?php echo $this->get_field_id($key); ?>"><?php ( $key == 'feed' ) ? _e('Feed url:', 'unspoken') : _e('Ссылка на профиль:', 'unspoken'); ?></label>
    ...
    function widget($args, $instance) {
        ...
        $title = empty($instance['title']) ? __('Оставайтесь на связи', 'unspoken') : apply_filters('widget_title', $instance['title']);
        echo $before_widget;
