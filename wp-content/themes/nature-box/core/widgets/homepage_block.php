...
class wpshower_homepage_block extends WP_Widget {
    ...
    function wpshower_homepage_block() {
        $widget_args = array('classname' => 'wpshower_homepage_block', 'description' => __('С помощью этого виджета можно настроить контент на главной странице.', 'unspoken') );
        parent::WP_Widget(false, __('Unspoken - Главная страница', 'unspoken'), $widget_args);
    }

    function form($instance) {
        $defaults = array(
            'title' => '',
            'slider_title' => 'Основной момент',
            'choice_title' => 'Выбор редакции',
            'video_title' => 'Видео',
            'type' => 'category',
            'category' => '',
            'category_perpage' => 5,
        ...
        <p>
            ...
            <select class="widefat" onchange="javascript: toggleType('<?php echo $this->get_field_id('type'); ?>');" name="<?php echo $this->get_field_name('type'); ?>" id="<?php echo $this->get_field_id('type'); ?>">
                <option <?php if ($instance['type'] == 'latest') { echo 'selected="selected"'; } ?> value="latest"><?php _e('Основной момент & Последние записи', 'unspoken'); ?></option>
                <option <?php if ($instance['type'] == 'category') { echo 'selected="selected"'; } ?> value="category"><?php _e('Рубрика', 'unspoken'); ?></option>
                <option <?php if ($instance['type'] == 'categories') { echo 'selected="selected"'; } ?> value="categories"><?php _e('Рубрики', 'unspoken'); ?></option>
                <option <?php if ($instance['type'] == 'choice') { echo 'selected="selected"'; } ?> value="choice"><?php _e('Выбор редакции', 'unspoken'); ?></option>
                <option <?php if ($instance['type'] == 'video') { echo 'selected="selected"'; } ?> value="video"><?php _e('Видео', 'unspoken'); ?></option>
            </select>
        ...
        <div class="latest">
            <p><label for="<?php echo $this->get_field_id('slider_title'); ?>"><?php _e( 'Заголовок слайдера:', 'unspoken' ); ?> <input class="widefat" id="<?php echo $this->get_field_id('slider_title'); ?>" name="<?php echo $this->get_field_name('slider_title'); ?>" type="text" value="<?php echo $slider_title; ?>" /></label></p>
            <p><label for="<?php echo $this->get_field_id('latest_perpage'); ?>"><?php _e( 'Количество записей в списке:', 'unspoken' ); ?> <input size="2" id="<?php echo $this->get_field_id('latest_perpage'); ?>" name="<?php echo $this->get_field_name('latest_perpage'); ?>" type="text" value="<?php echo $latest_perpage; ?>" /></label></p>
        </div>

        <div class="category">
            <p>
                <select class="widefat" name="<?php echo $this->get_field_name('category'); ?>" id="<?php echo $this->get_field_id('category'); ?>">
                    <option value=""><?php _e('Выберите рубрику', 'unspoken'); ?></option>
                    <?php
            ...
            <p><label for="<?php echo $this->get_field_id('category_perpage'); ?>"><?php _e( 'Количество записей в списке:', 'unspoken' ); ?> <input size="2" id="<?php echo $this->get_field_id('category_perpage'); ?>" name="<?php echo $this->get_field_name('category_perpage'); ?>" type="text" value="<?php echo $category_perpage; ?>" /></label></p>
        </div>

        <div class="categories">
            <p>
                <select class="widefat" name="<?php echo $this->get_field_name('categories1'); ?>" id="<?php echo $this->get_field_id('categories1'); ?>">
                    <option value=""><?php _e('Выберите рубрику, колонка 1', 'unspoken'); ?></option>
                    <?php
                    foreach ($categories as $catitem) {
                        $selected = '';
            ...
            <p>
                <select class="widefat" name="<?php echo $this->get_field_name('categories2'); ?>" id="<?php echo $this->get_field_id('categories2'); ?>">
                    <option value=""><?php _e('Выберите рубрику, колонка 2', 'unspoken'); ?></option>
                    <?php
                    foreach ($categories as $catitem) {
                        $selected = '';
            ...
            <p>
                <select class="widefat" name="<?php echo $this->get_field_name('categories3'); ?>" id="<?php echo $this->get_field_id('categories3'); ?>">
                    <option value=""><?php _e('Выберите рубрику, колонка 3', 'unspoken'); ?></option>
                    <?php
                    foreach ($categories as $catitem) {
                        $selected = '';
            ...
            <p>
                <select class="widefat" name="<?php echo $this->get_field_name('categories4'); ?>" id="<?php echo $this->get_field_id('categories4'); ?>">
                    <option value=""><?php _e('Выберите рубрику, колонка 4', 'unspoken'); ?></option>
                    <?php
                    foreach ($categories as $catitem) {
                        $selected = '';
            ...
            <p><label for="<?php echo $this->get_field_id('categories_perpage'); ?>"><?php _e( 'Количество записей в списке:', 'unspoken' ); ?> <input size="2" id="<?php echo $this->get_field_id('categories_perpage'); ?>" name="<?php echo $this->get_field_name('categories_perpage'); ?>" type="text" value="<?php echo $categories_perpage; ?>" /></label></p>
        </div>

        <div class="choice">
            <p><label for="<?php echo $this->get_field_id('choice_title'); ?>"><?php _e( 'Заголовок виджета:', 'unspoken' ); ?> <input class="widefat" id="<?php echo $this->get_field_id('choice_title'); ?>" name="<?php echo $this->get_field_name('choice_title'); ?>" type="text" value="<?php echo $choice_title; ?>" /></label></p>
        </div>

        <div class="video">
            <p><label for="<?php echo $this->get_field_id('video_title'); ?>"><?php _e( 'Заголовок виджета:', 'unspoken' ); ?> <input class="widefat" id="<?php echo $this->get_field_id('video_title'); ?>" name="<?php echo $this->get_field_name('video_title'); ?>" type="text" value="<?php echo $video_title; ?>" /></label></p>
            <p><label for="<?php echo $this->get_field_id('video_vis'); ?>"><?php _e( 'Число видимых видео:', 'unspoken' ); ?><br /><input size="2" id="<?php echo $this->get_field_id('video_vis'); ?>" name="<?php echo $this->get_field_name('video_vis'); ?>" type="text" value="<?php echo $video_vis; ?>" /></label></p>
            <p><label for="<?php echo $this->get_field_id('video_qty'); ?>"><?php _e( 'Максимальное количество видео:', 'unspoken' ); ?><br /><input size="2" id="<?php echo $this->get_field_id('video_qty'); ?>" name="<?php echo $this->get_field_name('video_qty'); ?>" type="text" value="<?php echo $video_qty; ?>" /></label> <span class="description"><?php _e( 'Установите -1, чтобы показать все', 'unspoken' ); ?></span></p>
        </div>
    ...
    function update( $new_instance, $old_instance ) {
        ...
        switch ( $instance['type'] ) {
            case 'latest' :
                $instance['title'] = __('Основные моменты & Последние записи', 'unspoken');
                break;
            case 'category' :
                $instance['title'] = __('Рубрика', 'unspoken');
                break;
            case 'categories' :
                $instance['title'] = __('Рубрики', 'unspoken');
                break;
            case 'choice' :
                $instance['title'] = __('Выбор редактора', 'unspoken');
                break;
            case 'video' :
                $instance['title'] = __('Видео', 'unspoken');
                break;
        }

        $instance['slider_title'] = ( strip_tags($new_instance['slider_title']) ) ? strip_tags($new_instance['slider_title']) : __( 'Основные моменты', 'unspoken' );
        $instance['choice_title'] = ( strip_tags($new_instance['choice_title']) ) ? strip_tags($new_instance['choice_title']) : __( 'Выбор редактора', 'unspoken' );
        $instance['video_title'] = ( strip_tags($new_instance['video_title']) ) ? strip_tags($new_instance['video_title']) : __( 'Видео', 'unspoken' );
        ...
        ?>
            ...
            <div class="latest clear">
                ...
                <div class="latest-news">
                    <div class="block-title"><?php _e('Последние Записи', 'unspoken'); ?></div>
                    <ul>
                    <?php
                        $args = array(
